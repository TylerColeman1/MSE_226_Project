...
NFL_Train_Total_Model.cv # RMSE= 6.5804 # May have collinearity
```
...
NFL_Train_Total_Model1.cv # RMSE= 6.5943 
``` 
...
plot(cvfit) 
bestlam = cvfit$lambda.min # = 10.098
```
...
# ridge_RMSE # = 6.5158
coef(ridge_mod)[,10.098] # Best is again basically forcing all the betas to 0. Just predict mean
...
plot(cvfit_lasso) 
bestlam_lasso = cvfit_lasso$lambda.min # = 0.10335
...
# CV won't work, RMSE calculation by hand will work
``` {r}
NFL_Train_Intuitive_Model.cv = cvFit(NFL_Train_Intuitive_Model, data=NFL_DATA_TRAIN_Filtered_Final, y=NFL_DATA_TRAIN_Filtered_Final$Yards, K=10, seed = 123)
NFL_Train_Intuitive_Model.cv # RMSE= 6.5943 
